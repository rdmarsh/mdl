class ServiceLevelsController < ApplicationController
  ...
  def create
    ...
    if @service_level.save
      flash[:notice] = "New service level '" + @service_level.name + "' created"
    else
  ...
  def update
    ...
    if @service_level.update_attributes(service_level_params)
      flash[:notice] = "Successfully updated service level '" + @service_level.name + "'"
    else
      render action: 'edit'
    end
    respond_with(@service_level)
  ...
  def destroy
    ...
    if @service_level.destroy
      flash[:notice] = "Successfully deleted service level '" + @service_level.name + "'"
    else
